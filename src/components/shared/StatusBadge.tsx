...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  STAFF: { label: "Staff", className: "bg-muted text-foreground border-border" },
  ADMIN: { label: "Admin", className: "bg-muted text-foreground border-border" },
  CUSTOMER: { label: "Customer", className: "bg-muted text-foreground border-border" },
};

export function StatusBadge({ status, icon: Icon }: StatusBadgeProps) {
  const config = statusConfig[status] || { label: status, className: "bg-muted text-foreground border-border" };
  return (
