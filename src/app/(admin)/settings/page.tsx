      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="w-full max-w-md border border-hairline bg-surface-soft p-8">
            ...
            <button onClick={handleSaveSlot} className="flex w-full items-center justify-center gap-2 bg-yellow-dark py-4 text-xs font-bold uppercase tracking-machined text-ink hover:bg-yellow-light">
              Save Slot
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="border border-hairline bg-surface-card p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-yellow-dark px-4 py-2 text-xs font-bold uppercase tracking-machined text-ink hover:bg-yellow-light transition-colors">
            ...
            {[...timeSlots].sort((a, b) => a.startTime.localeCompare(b.startTime)).map(slot => (
              <div key={slot.id} className="flex items-center justify-between border border-hairline bg-surface-soft p-4">
                <p className="text-sm font-bold text-ink flex items-center gap-3">
                  <Clock size={14} className="text-yellow-dark" /> 
                  {slot.label}
                </p>
        ...
        <div className="border border-hairline bg-surface-card p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="email" defaultValue="admin@estatecarwash.com" className="w-full bg-surface-soft border border-hairline text-ink p-4 text-sm font-light focus:border-yellow-dark focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-machined text-muted mb-2">Change Password</label>
              <input type="password" placeholder="••••••••" className="w-full bg-surface-soft border border-hairline text-ink p-4 text-sm font-light focus:border-yellow-dark focus:outline-none" />
            </div>
            <button className="w-full bg-yellow-dark py-4 text-xs font-bold uppercase tracking-machined text-ink hover:bg-yellow-light">
              Update Credentials
            </button>
          </div>
        </div>
